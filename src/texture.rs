...
pub fn get_texture(algorithm: TextureAlgorithm, x: i32, y: i32, z: i32, variants: u8) -> u8 {
    ...
    match algorithm {
        TextureAlgorithm::Vanilla1 => absolute_modulo(coordinate_random_legacy(x, y, z), variants),
        TextureAlgorithm::Vanilla2 => {
            absolute_modulo(random_vanilla2(coordinate_random(x, y, z)), variants)
        }
        TextureAlgorithm::Vanilla3 => legacy_next_int(coordinate_random(x, y, z), variants),
        TextureAlgorithm::Sodium1 => {
            absolute_modulo(random_sodium1(coordinate_random(x, y, z) as u64), variants)
        }
        TextureAlgorithm::Sodium2 => {
            absolute_modulo(random_sodium2(coordinate_random(x, y, z) as u64), variants)
        }
    }
}
