...
CF_HOST_DEVICE CF_FORCE_INLINE std::uint8_t absoluteModulo4(std::int32_t value)
{
    return static_cast<std::uint8_t>(std::llabs(value) & 3LL);
}
...
struct TextureSampler<TextureAlgorithm::Vanilla1> {
    ...
    {
        return texture_detail::absoluteModulo4(
