...
__host__ __device__ inline int64_t unsignedShiftRight(int64_t value, int distance)
{
    return static_cast<int64_t>(static_cast<uint64_t>(value) >> distance);
}

__host__ __device__ inline int64_t rotateLeft64(int64_t value, int distance)
{
    const uint64_t bits = static_cast<uint64_t>(value);
    return static_cast<int64_t>((bits << distance) | (bits >> (64 - distance)));
}

__host__ __device__ int positiveModulo(int value, char mod)
{
...
__host__ __device__ int64_t staffordMix13(int64_t z)
{
...
__host__ __device__ int64_t coordinateRandomRaw(int x, int y, int z)
{
...
__host__ __device__ int coordinateRandomLegacy(int x, int y, int z)
{
    return static_cast<int>(coordinateRandomRaw(x, y, z)) >> 16;
}

__host__ __device__ int64_t coordinateRandom(int x, int y, int z)
{
    return coordinateRandomRaw(x, y, z) >> 16;
}

__host__ __device__ int randomVanilla21_1(int64_t seed)
{
    seed = (seed ^ JavaMultiplier) & JavaMask;
    return static_cast<int>(unsignedShiftRight(seed * 0xBB20B4600A69LL + 0x40942DE6BALL, 16));
}

__host__ __device__ int64_t legacyScrambleSeed(int64_t seed)
{
    return (seed ^ JavaMultiplier) & JavaMask;
}

__host__ __device__ int legacyNextBits(int64_t& seed, int bits)
{
    seed = (seed * JavaMultiplier + 11LL) & JavaMask;
    return static_cast<int>(unsignedShiftRight(seed, 48 - bits));
}

__host__ __device__ int legacyNextInt(int64_t seed, char bound)
{
...
__host__ __device__ int randomSodium(int64_t seed)
{
...
__host__ __device__ int randomSodium19(int64_t seed)
{
...
__host__ __device__ char getTextureVanilla12(int x, int y, int z, char variantCount)
{
    return static_cast<char>(positiveModulo(coordinateRandomLegacy(x, y, z), variantCount));
}

__host__ __device__ char getTextureVanilla(int x, int y, int z, char variantCount)
{
    return static_cast<char>(legacyNextInt(coordinateRandom(x, y, z), variantCount));
}

__host__ __device__ char getTextureVanilla21_1(int x, int y, int z, char variantCount)
{
    return static_cast<char>(positiveModulo(randomVanilla21_1(coordinateRandom(x, y, z)), variantCount));
}

__host__ __device__ char getTextureSodium(int x, int y, int z, char variantCount)
{
    return static_cast<char>(positiveModulo(randomSodium(coordinateRandom(x, y, z)), variantCount));
}

__host__ __device__ char getTextureSodium19(int x, int y, int z, char variantCount)
{
    return static_cast<char>(positiveModulo(randomSodium19(coordinateRandom(x, y, z)), variantCount));
}
}
